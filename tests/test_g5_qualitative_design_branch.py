...
from run_g5_auto import build_redcap_rows, build_strobe_flowchart, gen_data_cleaning_script  # noqa: E402
...
class TestQualitativeCRFIsNotQuantitativeClinicalForm:
    ...
    def test_qualitative_crf_has_sampling_and_saturation_fields(self):
        rows, _ = build_redcap_rows("qualitative", _TOPIC)
        names = [r[0] for r in rows]
        for f in ("record_id", "sampling_category", "coding_round", "saturation_reached"):
    ...
    def test_qualitative_crf_date_fields_use_date_ymd_validation_type(self):
        """Hồi quy HIGH (vòng lặp kiểm tra-hoàn thiện vòng 3, 2026-07-21):
        recruitment_date/interview_date trước đây dùng validation_type "date"
        (không hợp lệ) thay vì "date_ymd" chuẩn của toàn bộ file — khiến
        DATE_COLS trong script làm sạch tự sinh bỏ sót 2 cột này."""
        rows, _ = build_redcap_rows("qualitative", _TOPIC)
        by_name = {r[0]: r for r in rows}
        assert by_name["recruitment_date"][7] == "date_ymd"
        assert by_name["interview_date"][7] == "date_ymd"

    def test_qualitative_crf_uses_record_id_not_participant_id(self):
        """Hồi quy MEDIUM (vòng lặp kiểm tra-hoàn thiện vòng 3, 2026-07-21):
        gen_data_cleaning_script()::check_duplicates() (và báo cáo dữ liệu
        thiếu) dò CỨNG cột tên 'record_id' — dùng tên khác ('participant_id')
        làm các kiểm tra đó ÂM THẦM không chạy gì."""
        rows, _ = build_redcap_rows("qualitative", _TOPIC)
        names = [r[0] for r in rows]
        assert "record_id" in names
        assert "participant_id" not in names

    def test_qualitative_generated_cleaning_script_checks_duplicates(self):
        rows, _ = build_redcap_rows("qualitative", _TOPIC)
        script = gen_data_cleaning_script("STUDY-X", "qualitative", rows)
        assert '"record_id" in df.columns' in script
