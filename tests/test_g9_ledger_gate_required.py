...
def _configure_test_signing_key(tmp_path: Path, monkeypatch) -> None:
    ...
    key_path.write_text("pytest-g9-key", encoding="utf-8", newline="\n")
...
def _write_artifacts_and_locked_text(study: str, d: Path) -> tuple[str, str]:
    ...
    (d / f"G2_A3_ETHICS_PACKAGE_{study}.md").write_text(g2_content, encoding="utf-8", newline="\n")
    (d / f"G4_A5_SAP_FINAL_{study}.md").write_text(g4_content, encoding="utf-8", newline="\n")
