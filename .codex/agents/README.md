...
> 🗺️ **Bản đồ kết nối toàn đội (điểm vào · luồng · điểm cuối · cầu nối · hub):** `.Codex/agents/_BAN-DO-KET-NOI.md`.
...
- **Đồng bộ Mac:** `.Codex/agents/` ở trong OneDrive → tự sync; trên MacBook mở `claude` trong `~/OneDrive/Claude AI` là có cùng đội agent. (Đợi OneDrive xanh trước khi đổi máy; bật "Always keep on this device" cho chắc.)
...
Mỗi routine có khối **AGENT WIRING (2026-06-12)** ở cuối — cộng thêm, không sửa prompt gốc. Phiên headless không gọi được subagent → routine tự làm theo đặc tả `.Codex/agents/<tên>.md` (cùng chuẩn).
