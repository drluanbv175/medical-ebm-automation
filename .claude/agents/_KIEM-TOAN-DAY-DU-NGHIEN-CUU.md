...
11. **HỢP ĐỒNG CHUẨN HIỆN HÀNH PHẢI CHẠY ĐƯỢC:** không chấm ✅ chỉ vì tài liệu có nhắc tên chuẩn. G2 phải fail-closed khi WHO TRDS 1.3.1 mục 13/14/19/20 thiếu dữ kiện PI đã pin hoặc tham chiếu Hội đồng chỉ là fallback; G9 phải fail-closed khi thiếu quyền rà dữ liệu của mọi tác giả, tác giả truy cập dữ liệu gốc/tham gia phân tích, hoặc bằng chứng độc lập với nhà tài trợ theo ICMJE 1/2026. Chạy `python tools/verify_controlled_research_automation.py` từ thư mục gốc để kiểm hồi quy hợp đồng này.
...
| A3 | **Hồ sơ đạo đức (IRB) + Phiếu đồng thuận (ICF)** | **G2** | `dao-duc-dang-ky` | **bắt buộc TRƯỚC thu dữ liệu**; tham chiếu Hội đồng rõ nguồn, không fallback |
| A4 | Đăng ký nghiên cứu | G2 | `dao-duc-dang-ky` | WHO TRDS 1.3.1 đủ 24 mục; 13/14/19/20 lấy từ G0/G1 do PI pin; bắt buộc cho thử nghiệm; quan sát → nêu quyết định có/không + lý do |
...
| A14 | Khai báo COI · tài trợ · đóng góp tác giả · dùng AI · quyền truy cập dữ liệu | G9 | `nop-bai-phan-hoi` (soạn) + `binh-duyet` (rà) | ICMJE 1/2026: mọi tác giả rà được dữ liệu hỗ trợ; chỉ rõ người truy cập dữ liệu gốc/tham gia phân tích; bảo toàn độc lập với nhà tài trợ; **chủ nhiệm xác nhận** |
...
- **Chẩn đoán (độ chính xác):** **STARD/STARD-AI**; **QUADAS-3 hiện hành** (QUADAS-2 chỉ tương thích ngược; QUADAS-C khi phù hợp); reference standard; **Se/Sp/PPV/NPV/LR/AUC**; ngưỡng cắt; GRADE cho test → `tham-dinh-do-chinh-xac-chan-doan`.
