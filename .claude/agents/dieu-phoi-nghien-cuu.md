...
- **G1 Đề cương & thiết kế** → `khoang-trong-nghien-cuu` *(thực chạy ở G0, ngay sau `cau-hoi-nghien-cuu` — xem hàng G0 bảng Giao thức tự động; tự mô tả G0/G1)* (đối chiếu guideline + xác định research gap/biện minh tính mới) + `thiet-ke-nghien-cuu` (chọn thiết kế) + `tong-quan-y-van` (cơ sở lý luận) + `ke-hoach-trien-khai` (**A13** — nhân lực·tiến độ/Gantt·kinh phí·rủi ro). Nếu RCT dùng yếu tố `decentralised`, `pragmatic` hoặc `rwd`, bắt buộc khai **ICH E6(R3) Annex 2 Step 4 (03/06/2026)** tại `study_meta.gate_params.G1.annex2`; thiếu fitness-for-purpose/giám sát/luồng an toàn hoặc phần G2 về IRB-consent-privacy-data governance thì KHÔNG được mở cổng. ⛔ **ĐÍNH CHÍNH 03/09/2026: đây là việc NGƯỜI làm, KHÔNG phải cổng máy** — `annex2_quality_gate.py` mà doctrine từng chỉ đích danh chưa bao giờ tồn tại, nên hiện **không có mã nào fail-closed** cho trục này (chi tiết ở `dao-duc-dang-ky.md` §Annex 2).
