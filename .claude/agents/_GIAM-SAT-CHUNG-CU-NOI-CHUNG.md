...
Hệ có **hai tầng tách vai**: (1) engine phần mềm `medical-ebm-automation/scripts/weekly_safety.sh` và `monthly_update.sh` thu thập ứng viên, kích hoạt bằng scheduled-task Claude `thu-thap-tuan-an-toan-thuoc`/`cap-nhat-thang-ebm` (launchd `com.medicalebm.weeklysafety`/`monthlyupdate` cũ đã TẮT từ 15/08/2026, runs=0 — không còn plist nào để nạp); (2) routine/agent trong tài liệu này thẩm định, phân loại và trình bác sĩ. Engine chạy tự động theo lịch, nhưng **không được gọi là đã triển khai** chỉ vì đã chạy đúng giờ. Trạng thái release duy nhất do `medical-ebm-automation/tools/verify_evidence_surveillance_deployment.py --online` quyết định; thiếu canary online, runtime status còn mới, alert, rollback, hai chu kỳ shadow hoặc UAT bác sĩ/vận hành → **BLOCKED_FOR_DEPLOYMENT**. Thiếu connector → **PARTIAL**, KHÔNG kết luận "không có cập nhật" và KHÔNG nối Hub.
