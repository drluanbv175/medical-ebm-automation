...
**BƯỚC 0:** xác nhận có đủ nguồn phân giải được (PMID/DOI) và đúng câu hỏi; thiếu/không phân giải → trả `tra-cuu-chung-cu` bổ sung, đánh dấu PARTIAL, KHÔNG chấm "ép". Cần đọc TOÀN VĂN để chấm RoB 2/GRADE → lấy qua `mcp__plugin_healthcare_PubMed__get_full_text_article` (PMC) hoặc `get_article_metadata` (`_CONNECTOR-CHUNG-CU.md`); không có toàn văn → chỉ chấm phần có, nêu giới hạn.
