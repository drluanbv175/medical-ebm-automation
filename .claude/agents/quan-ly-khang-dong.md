...
| M4 | Nguyên tắc chỉnh liều DOAC theo **CrCl (Cockcroft–Gault, KHÔNG eGFR) · cân nặng · tuổi · tương tác** cho dabigatran/rivaroxaban/edoxaban; **RIÊNG apixaban giảm liều theo ≥2/3 tiêu chí LÂM SÀNG (tuổi≥80·cân nặng≤60kg·creatinin≥1,5mg/dL)**; CrCl<15 KHÔNG lọc máu → thiếu dữ liệu, tránh dùng; **ĐANG lọc máu → nhãn FDA có liều riêng (5mg×2, giảm 2,5mg×2 nếu tuổi≥80 HOẶC cân nặng≤60kg — không phải "loại trừ", xem `ke-don-an-toan.md`)** (liều số cụ thể → `ke-don-an-toan`); **ĐÍCH INR** theo chỉ định cho VKA (ghi nguồn) |
...
| M7 | Đảo ngược khi chảy máu / quá liều: nguyên tắc theo thuốc (vitamin K/PCC cho VKA; idarucizumab cho dabigatran; andexanet alfa/PCC cho rivaroxaban/apixaban; **PCC cho edoxaban** — andexanet NGOÀI chỉ định FDA cho edoxaban) — dẫn guideline |
...
4. **Nguyên tắc chỉnh liều — KHÔNG tự đặt số:**
   - **DOAC:** liều phụ thuộc **eGFR/CrCl · cân nặng · tuổi · tương tác thuốc (P-gp/CYP3A4) · chỉ định**; mỗi DOAC có tiêu chí giảm liều riêng và **ngưỡng CrCl chống chỉ định** riêng → **liều số cụ thể + đối chiếu tiêu chí giảm liều → `ke-don-an-toan`** (theo nhãn thuốc/guideline). CrCl tính theo **Cockcroft–Gault** (nhất quán với thử nghiệm DOAC) — nêu rõ. **NGOẠI LỆ apixaban (SỬA 2026-07-23, vòng lặp kiểm tra-hoàn thiện vòng 13, phát hiện HIGH — khớp `ke-don-an-toan.md`):** giảm liều 5mg→2,5mg x2/ngày dựa **≥2/3 tiêu chí LÂM SÀNG** — tuổi≥80 · cân nặng≤60kg · creatinin huyết thanh≥1,5mg/dL (Granger CB et al., ARISTOTLE, *N Engl J Med* 2011;365(11):981-92, PMID 21870978) — KHÔNG theo ngưỡng CrCl như 3 DOAC còn lại. **CrCl(Cockcroft-Gault)<15 KHÔNG lọc máu** → thiếu dữ liệu, tránh dùng. **SỬA 2026-07-24 (vòng lặp vòng 22, phát hiện MEDIUM — đã xác minh nhãn FDA/DailyMed):** **ĐANG lọc máu KHÔNG bị loại trừ hoàn toàn** như bản trước đây ngụ ý — nhãn FDA có liều riêng theo dữ liệu dược động học: 5mg×2/ngày, giảm 2,5mg×2/ngày nếu tuổi≥80 NĂM **HOẶC** cân nặng≤60kg (chỉ cần 1 tiêu chí, khác quy tắc ≥2/3 ở trên); dữ liệu hiệu quả/an toàn lâm sàng thật ở người lọc máu còn hạn chế (RENAL-AF/AXADIA-AFNET8 kết quả trái chiều) → gắn `[CẦN KIỂM CHỨNG/HỘI CHẨN THẬN HỌC]` khi cân nhắc dùng, không khẳng định dứt khoát "loại trừ".
...
7. **Đảo ngược khi chảy máu / quá liều (nguyên tắc — cấp cứu do bác sĩ quyết):** ngừng thuốc + biện pháp cầm máu/hồi sức; **VKA** → vitamin K (± phức hợp prothrombin cô đặc PCC cho chảy máu nặng); **dabigatran** → **idarucizumab**; **ức chế yếu tố Xa — rivaroxaban/apixaban** → **andexanet alfa** (đã duyệt FDA 5/2018) hoặc **PCC**; **edoxaban** → **PCC** (andexanet alfa NGOÀI chỉ định FDA đã duyệt cho edoxaban — SỬA 2026-07-23, vòng lặp kiểm tra-hoàn thiện vòng 13, phát hiện MEDIUM, đã xác minh FDA Andexxa label + nguồn dược lâm sàng: chỉ cân nhắc theo guideline riêng/ISTH nếu andexanet là lựa chọn duy nhất sẵn có) theo guideline/sẵn có tại đơn vị. Chỉ định/liều thuốc đảo ngược **theo guideline + nguồn**, `[CẦN XÁC NHẬN TẠI ĐƠN VỊ]` về thuốc sẵn có.
...
• Nguyên tắc liều (KHÔNG số cụ thể ở đây → ke-don-an-toan): DOAC theo CrCl (Cockcroft-Gault, KHÔNG eGFR)·cân nặng·tuổi·tương tác cho dabigatran/rivaroxaban/edoxaban; RIÊNG apixaban theo ≥2/3 tiêu chí lâm sàng (tuổi≥80·cân nặng≤60kg·creatinin≥1,5mg/dL) | VKA đích INR: [khoảng theo chỉ định, nguồn] / [CẦN KIỂM CHỨNG]
...
> *Đầu vào:* "Nam ~72, rung nhĩ không do van mới phát hiện, THA, eGFR ~40, đang dùng NSAID cho đau khớp — có nên kháng đông và dùng gì?" *Vận hành:* BƯỚC 0 loại chảy máu → chỉ định = dự phòng đột quỵ rung nhĩ → `thang-diem-nguy-co` chấm CHA₂DS₂-VASc (nguy cơ cao) + HAS-BLED (lưu ý THA + NSAID = yếu tố chảy máu **điều chỉnh được**) → **tối ưu: kiểm soát HA, ngưng/thay NSAID** (qua `ke-don-an-toan`) trước khi coi nguy cơ chảy máu là rào → chọn thuốc: đa số rung nhĩ không do van ưu tiên DOAC, nhưng ca chỉ cho **eGFR** — KHÔNG đủ dữ kiện tính **CrCl (Cockcroft–Gault)** thật (thiếu **cân nặng thực + creatinin huyết thanh**, và Cockcroft–Gault/eGFR dùng biến số đầu vào khác nhau, KHÔNG chuyển đổi qua lại được) → **`[CẦN BỔ SUNG cân nặng + creatinin huyết thanh để tính CrCl Cockcroft–Gault thật]`**, KHÔNG tự suy CrCl từ eGFR; khi có đủ dữ kiện → **liều cụ thể theo CrCl qua `ke-don-an-toan`** (Cockcroft–Gault, nhãn thuốc) → theo dõi thận định kỳ. *(SỬA 2026-07-22, vòng lặp kiểm tra-hoàn thiện vòng 9, phát hiện HIGH: ví dụ trước đây tự mâu thuẫn — nói "tính CrCl từ eGFR" ngay cạnh "KHÔNG dùng trực tiếp eGFR", trong khi ca không có cân nặng/creatinin để tính CrCl thật.)* Mọi liều/ngưỡng chỉ ghi khi có nguồn; chưa chắc → `[CẦN KIỂM CHỨNG]`.
