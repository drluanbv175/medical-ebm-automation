...
- `lib/workflow-actions.ts`: adds persistent write plans for `approveCarePlanVersionAction` and `releaseApprovedPatientHandoutAction`, using `buildPersistentAuditWritePlan` while keeping production commit disabled until atomic DB tests exist.
...
2. Wire `approveCarePlanVersionAction` and `releaseApprovedPatientHandoutAction` to a real database transaction only after tests prove the AuditLog insert and business write commit atomically.
3. Keep every other server action preview-only until it has the same persistent audit write plan and atomic transaction tests.
