...
Production can be declared only by a valid evidence package loaded through `PRODUCTION_READINESS_EVIDENCE_PATH`. The package must clear all 24 blockers, include all seven independent signoffs, include a final approval record from a distinct clinic admin approver, and pass `pnpm production:dossier` so every blocker evidence record names the required repository runtime control IDs. Final `pnpm production:go-live` also requires dual-control admin change attestation, rollback plan and post-deployment checklist. See `PRODUCTION_GO_LIVE_RUNBOOK.md`.
