...
| 9.6 | **[Phát hiện phụ, ngoài kế hoạch ban đầu]** `WebDashboard_EBM_VanDeCuThe_ViemKhopDangThap_20260610.html` ITEM-07: PMID 36357155 xuất bản THẬT năm 2023 nhưng item khai `dateVersion=2025`/`2021` — nghi trích dẫn nhầm phiên bản/năm của cùng họ guideline; `verify_dashboard.py --strict-sources` báo lỗi cứng. Phát hiện tình cờ khi xác minh 9.2 không ảnh hưởng nội dung — đây là lỗi TRÍCH DẪN có sẵn từ trước, chưa rõ diện rộng (đang quét toàn kho 67 dashboard chạy nền). KHÔNG tự đổi `decision`/`gradeLevel`/năm trích dẫn — chỉ đo diện rộng rồi báo bác sĩ. | Báo cáo số dashboard/mục bị ảnh hưởng kèm trích nguyên văn; không tự sửa nội dung y khoa | — | `cc:wip` — đang quét `--online --strict-sources` toàn kho 67 dashboard |
